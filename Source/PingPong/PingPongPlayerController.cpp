...
{
	if (Platform && Platform->Ball)																		//Проблема была в этом месте. Если я пишу этот код через обычный метод Fire, то Platform NULLPTR.
	{																									//Но если я использую RPC Server_Fire, то все работает исправно, платформа, как и в методе 
		GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Green, FString::Printf(TEXT("TEST")));			//ServerPlatformMove_Right на месте и не равна NULLPTR. Почему это так работает?
		Platform->Ball->IsMoving = true;																//Все это справедливо для второго PlayerController, первый при любом раскладе работает исправно.
